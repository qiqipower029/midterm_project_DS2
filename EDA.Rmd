...
The final MARS model retained 7 predictors and no interaction term. The predictors are: `sqft_living`, `grade`,`condition`, `yr_built`, `sqft_living15`, `waterfront`, and `sqft_lot15`. 
...
p5 <- partial(mars.fit, pred.var = c("condition"), grid.resolution = 10) %>% autoplot()
...
set.seed(1)
resamp = resamples(list(lasso = lasso.fit, ridge = ridge.fit, 
                        lm = model_ls,
                        mars = mars.fit,
                        gam = gam.fit.caret))
summary(resamp)
